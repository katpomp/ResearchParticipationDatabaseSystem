...
// Fetch all studies
$studies = [];
$result = $conn->query("SELECT StudyID, StudyTitle, StartDate, ResearcherID FROM Study ORDER BY StartDate ASC");
...
$signups = [];
$stmt = $conn->prepare("SELECT SP.StudyID, S.StudentID, S.FirstName, S.LastName, S.Email 
...
.container{display:flex;gap:20px;padding:20px 2rem 0 2rem;margin-bottom:20px;}
...
.preview-item{margin-bottom:12px;padding:12px;border:1px solid #d9dfe7;border-left:4px solid var(--cnu-blue);border-radius:8px;background:#fafbfd;}
.preview-title{font-weight:600;color:var(--text-dark);margin-bottom:4px;}
.preview-meta{color:#55626e;font-size:0.92rem;}
.study-action{text-align:center;margin-top:8px;}
.action-link{
    display:inline-block;
    text-decoration:none;
    background:var(--cnu-blue);
...
<div class="top-tabs">
    <a href="faculty_dashboard.php" class="top-tab-link active">&#8962; Home</a>
...
<div class="container">
    ...
    <div class="right-column">
        <div class="panel">
            <a href="faculty_studies.php" class="sidebar-header-link">All Studies & Signups</a>
            <?php foreach(array_slice($studies, 0, 4) as $study): ?>
                <div class="preview-item">
                    <div class="preview-title"><?php echo htmlspecialchars($study['StudyTitle']); ?></div>
                    <div class="preview-meta"><?php echo htmlspecialchars(date('M j, Y', strtotime($study['StartDate']))); ?></div>
                    <div class="preview-meta">Signups: <?php echo htmlspecialchars((string)count($signups[$study['StudyID']] ?? [])); ?></div>
                </div>
            <?php endforeach; ?>
            <div class="study-action">
                <a href="faculty_studies.php" class="action-link">Open Full Studies View</a>
            </div>
        </div>

        <div class="panel">
            <a href="faculty_directory.php" class="sidebar-header-link">Researchers & Students</a>
            <div class="preview-item">
                <div class="preview-title">View full people directory</div>
                <div class="preview-meta">Browse all researchers and students in one place.</div>
            </div>
            <div class="study-action">
                <a href="faculty_directory.php" class="action-link">Open Directory</a>
            </div>
        </div>
    </div>
</div>
